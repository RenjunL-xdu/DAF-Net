...
def wbf(bbreg_samples, samples, scores, ishape):
    bbreg_samples[:, 2:] = bbreg_samples[:, :2] + bbreg_samples[:, 2:]
    samples[:, 2:] = samples[:, :2] + samples[:, 2:]
    bbreg_samples[:, [0, 2]] /= ishape[1]
    samples[:, [0, 2]] /= ishape[1]
    bbreg_samples[:, [1, 3]] /= ishape[0]
    samples[:, [1, 3]] /= ishape[0]
    # if np.any(samples>1):
    #     print(1)
    # if np.any(bbreg_samples>1):
    #     print(1)
    boxes_list = [bbreg_samples, samples]
    scores = [float(i) for i in scores / scores.max()]
    scores_list = [scores,
                   scores]
    ...
    boxes, scores, labels = weighted_boxes_fusion(boxes_list, scores_list, labels_list, weights=weights,
                                                  iou_thr=0.6, skip_box_thr=0.0)
    boxes[:, [0, 2]] *= ishape[1]
    boxes[:, [1, 3]] *= ishape[0]
    boxes[:, 2:] = boxes[:, 2:] - boxes[:, :2]
    if len(boxes) != 1:
        boxes = boxes.mean(axis=0)
    return boxes
    ...
    for iter in range(maxiter):
        ...
        if batch_neg_cand > batch_neg:
            ...
            for start in range(0, batch_neg_cand, batch_test):
                ...
                score = model(batch_neg_feats[start:end].view(-1, 512, 3, 3),
                              batch_neg_feats[start:end].view(-1, 512, 3, 3), in_layer=in_layer, mode="test")
                if start == 0:
        ...
        # forward
        pos_score = model(batch_pos_feats.view(-1, 512, 3, 3), batch_pos_feats.view(-1, 512, 3, 3),
                          in_layer=in_layer, mode='test')
        neg_score = model(batch_neg_feats.view(-1, 512, 3, 3), batch_neg_feats.view(-1, 512, 3, 3),
                          in_layer=in_layer, mode='test')

        # optimize
        loss = criterion(pos_score, neg_score)
        # loss_rgb = criterion(pos_rgb, neg_rgb)
        # loss_ir=criterion(pos_ir, neg_ir)
...
def run_mdnet(img_list_v, img_list_t, init_bbox, gt=None, savefig_dir='', display=False,
              topk=[0, 0, 0], scale_large=True):
    ############################################
    ############################################
    ############################################
    # Init bbox
    ...
    result_best = np.zeros((len(img_list_t), 4))
    result_mean = np.zeros((len(img_list_t), 4))
    result_wbf = np.zeros((len(img_list_t), 4))
    result[0] = np.copy(target_bbox)
    result_bb[0] = np.copy(target_bbox)
    result_best[0] = np.copy(target_bbox)
    result_mean[0] = np.copy(target_bbox)
    result_wbf[0] = np.copy(target_bbox)

    iou_result = np.zeros((len(img_list_t), 1))
    dis_result = np.zeros((len(img_list_t), 1))
    # execution time array
    exec_time_result = np.zeros((len(img_list_t), 1))

    # Init model
    model = MDNet(opts['model_path_all'], K=1, topK=topk)
    ...
    model.set_learnable_params(model, opts['ft_layers'], False)
    ...
    for bidx in range(0, scene_boxes.shape[0]):
        ...
        cropped_image_v, cur_image_var_v = img_crop_model.crop_image(cur_image_v, np.reshape(scene_boxes[bidx], (1, 4)),
                                                                     crop_img_size)

        cropped_image_v = cropped_image_v - 128.
        cropped_image_t, cur_image_var_t = img_crop_model.crop_image(cur_image_t, np.reshape(scene_boxes[bidx], (1, 4)),
                                                                     crop_img_size)
        ...
        cur_pos_feats_mid = model.roi_align_model_mid(feat_map, cur_pos_rois)
        cur_pos_feats_high = model.roi_align_model_high(feat_map, cur_pos_rois)
        cur_pos_feats = model.mutil_fusion(cur_pos_feats, cur_pos_feats_mid, cur_pos_feats_high)
        ...
        cur_neg_feats_mid = model.roi_align_model_mid(feat_map, cur_neg_rois)
        cur_neg_feats_high = model.roi_align_model_high(feat_map, cur_neg_rois)
        cur_neg_feats = model.mutil_fusion(cur_neg_feats, cur_neg_feats_mid, cur_neg_feats_high)
        ...
        cur_bbreg_feats_mid = model.roi_align_model_mid(feat_map, cur_bbreg_rois)
        cur_bbreg_feats_high = model.roi_align_model_high(feat_map, cur_bbreg_rois)
        cur_bbreg_feats = model.mutil_fusion(cur_bbreg_feats, cur_bbreg_feats_mid, cur_bbreg_feats_high)
    ...
    for iidx in range(replicateNum):
        ...
        cur_extra_cropped_image_v, _ = img_crop_model.crop_image(cur_image_v, np.reshape(extra_scene_box, (1, 4)),
                                                                 extra_crop_img_size)
        cur_extra_cropped_image_v = cur_extra_cropped_image_v.detach()
        cur_extra_cropped_image_t, _ = img_crop_model.crop_image(cur_image_t, np.reshape(extra_scene_box, (1, 4)),
                                                                 extra_crop_img_size)
    ...
    extra_pos_feats_mid = model.roi_align_model_mid(extra_feat_maps, extra_pos_rois)
    extra_pos_feats_high = model.roi_align_model_high(extra_feat_maps, extra_pos_rois)
    extra_pos_feats = model.mutil_fusion(extra_pos_feats, extra_pos_feats_mid, extra_pos_feats_high)
    ...
    extra_neg_feats_mid = model.roi_align_model_mid(extra_feat_maps, extra_neg_rois)
    extra_neg_feats_high = model.roi_align_model_high(extra_feat_maps, extra_neg_rois)
    extra_neg_feats = model.mutil_fusion(extra_neg_feats, extra_neg_feats_mid, extra_neg_feats_high)
    ...
    extra_bbreg_feats_mid = model.roi_align_model_mid(extra_feat_maps, extra_bbreg_rois)
    extra_bbreg_feats_high = model.roi_align_model_high(extra_feat_maps, extra_bbreg_rois)
    extra_bbreg_feats = model.mutil_fusion(extra_bbreg_feats, extra_bbreg_feats_mid, extra_bbreg_feats_high)
    ...
    if bbreg_feats.size(0) > opts['n_bbreg']:
        ...
        bbreg_feats = bbreg_feats[bbreg_idx[0:opts['n_bbreg']], :]
        bbreg_examples = bbreg_examples[bbreg_idx[0:opts['n_bbreg']], :]
    bbreg = BBRegressor((ishape[1], ishape[0]), scale_large=scale_large)
    ...
    for i in range(1, len(img_list_v)):
        ...
        samples = gen_samples(SampleGenerator('gaussian', (ishape[1], ishape[0]), trans_f, opts['scale_f'], valid=True),
                              target_bbox, opts['n_samples'])  # 根据首帧采样256个样本
        ...
        crop_img_size = (padded_scene_box[2:4] * ((opts['img_size'], opts['img_size']) / target_bbox[2:4])).astype(
            'int64')
        cropped_image_v, cur_image_var_v = img_crop_model.crop_image(cur_image_v, np.reshape(padded_scene_box, (1, 4)),
                                                                     crop_img_size)
        # array = cropped_image_v.squeeze(0).permute(1, 2, 0).detach().cpu().numpy()
        # image = Image.fromarray(np.uint8(array))
        # image.save(f'pic/{i}.png')
        cropped_image_v = cropped_image_v - 128.
        cropped_image_t, cur_image_var_t = img_crop_model.crop_image(cur_image_t, np.reshape(padded_scene_box, (1, 4)),
                                                                     crop_img_size)
        cropped_image_t = cropped_image_t - 128.
        model.eval()
        feat_map = model(cropped_image_v, cropped_image_t, out_layer='conv3')

        # feat_map1 = feat_map.permute(0, 2, 3, 1)  # 将通道维度移到最后
        # 遍历每个通道
        # for id, channel in enumerate(range(  512*2)):
        #     # 获取指定通道的图像
        #     heatmap = feat_map1[0, :, :, channel]
        #
        #     # 使用 viridis 色彩映射绘制图像
        #     # plt.matshow(heatmap.cpu(), cmap='viridis')
        #     import cv2
        #     img = cv2.imread(f'pic/{i}.png')
        #     heatmap = cv2.resize(np.array(heatmap.cpu()), (img.shape[1], img.shape[0]))
        #     heatmap = np.uint8(10 * heatmap)
        #     heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
        #     superimposed_img = heatmap * 0.5 + img
        #     cv2.imwrite(f'pic/heatmap_{id}.jpg', superimposed_img)
        ...
        sample_feats_mid = model.roi_align_model_mid(feat_map, sample_rois)
        sample_feats_high = model.roi_align_model_high(feat_map, sample_rois)
        sample_feats=model.mutil_fusion(sample_feats,sample_feats_mid,sample_feats_high).view(-1, 512*3* 3)
        sample_scores = model(sample_feats.view(-1, 512, 3, 3), sample_feats.view(-1, 512, 3, 3),
                              in_layer='fc4', mode='test')
        top_scores, top_idx = sample_scores[:, -1].topk(5)  # 取出(256,2)中前景的前5个得分
        # print(i," : ",top_scores[0])
        # if(top_scores[0]<-1):lll
        #     print(1)
        top_idx = top_idx.data.cpu().numpy()  # 计算最高的5个样本的id
        target_score = top_scores.data.mean()  # 计算最高的5个样本的得分
        target_bbox = samples[top_idx].mean(axis=0)  # 取出(  512*2,4)中分数最高的5个
        ...
        if success:
            ...
            result_wbf[i] = wbf(bbreg_samples, samples[top_idx], top_scores.data, ishape)
        else:
            bbreg_bbox = target_bbox
            result_wbf[i] = bbreg_bbox

        # Save result
        result[i] = target_bbox
        result_bb[i] = bbreg_bbox
        result_mean[i] = np.vstack((bbreg_bbox, target_bbox)).mean(axis=0)

        ##########################################################
        if overlap_ratio(gt[i], result_bb[i])[0] < overlap_ratio(gt[i], result[i])[0]:
            result_best[i] = result[i]
        else:
            result_best[i] = result_bb[i]
        ##########################################################
            ...
            for bidx in range(0, scene_boxes.shape[0]):
                crop_img_size = (scene_boxes[bidx, 2:4] * (
                        (opts['img_size'], opts['img_size']) / target_bbox[2:4])).astype('int64') * jitter_scale[
                                    bidx]
                cropped_image_v, cur_image_var_v = img_crop_model.crop_image(cur_image_v,
                                                                             np.reshape(scene_boxes[bidx], (1, 4)),
                                                                             crop_img_size)
                cropped_image_v = cropped_image_v - 128.
                cropped_image_t, cur_image_var_t = img_crop_model.crop_image(cur_image_t,
                                                                             np.reshape(scene_boxes[bidx], (1, 4)),
                                                                             crop_img_size)
                cropped_image_t = cropped_image_t - 128.
                feat_map = model(cropped_image_v, cropped_image_t, out_layer='conv3')
        ...
        if opts['visual_log']:
            if gt is None:
                print("Frame %d/%d, Score %.3f, Time %.3f" % \
                      (i, len(img_list_v), target_score, spf))
            else:
                print("Frame %d/%d, Overlap %.3f, Score %.3f, Time %.3f" % \
                      (i, len(img_list_v), overlap_ratio(gt[i], result_bb[i])[0], target_score, spf))
        iou_result[i] = overlap_ratio(gt[i], result_best[i])[0]
        dis_result[i] = dis_ratio(gt[i], result_best[i])[0]
        # iou_result[i] = overlap_ratio(gt[i], result_bb[i])[0]
        # dis_result[i] = dis_ratio(gt[i], result_bb[i])[0]

    fps = len(img_list_v) / spf_total
    return iou_result, result_bb, fps, result, dis_result, result_mean, result_wbf, result_best
